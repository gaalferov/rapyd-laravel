...
use Iginikolaev\Rapyd\DataSet as DataSet;
use Iginikolaev\Rapyd\Persistence;
...
class DataGrid extends DataSet
{
    ...
    public function build($view = '')
    {
        ...
        $this->output = \View::make($view, array(
            'dg' => $this,
            'buttons' => $this->button_container,
            'label' => $this->label,
        ))->render();
    ...
    public function buildCSV($file = '', $timestamp = '', $sanitize = true, $del = array())
    {
        ...
        fputs($handle,
            $delimiter['enclosure'] . implode($delimiter['enclosure'] . $delimiter['delimiter'] . $delimiter['enclosure'],
        ...
        foreach ($this->data as $tablerow) {
            ...
            foreach ($this->columns as $column) {
                ...
                $cell = new Cell($column->name);
                $value = str_replace('"', '""', str_replace(PHP_EOL, '',
                    strip_tags($this->getCellValue($column, $tablerow, $sanitize))));
    ...
    protected function getCellValue($column, $tablerow, $sanitize = true)
    {
        ...
        } elseif (preg_match('#^[a-z0-9_-]+(?:\.[a-z0-9_-]+)+$#i', $column->name,
                $matches) && is_object($tablerow)
        ) {
        ...
        } elseif ($column->name instanceof \Closure) {
            dd($column);
        ...
        if ($column->route) {
            $key = isset($tablerow[$column->name]) ?  $tablerow[$column->name] : $column->label;
            $value = '<a href="' . route($column->route, $tablerow) . '">' . $key . '</a>';
        } elseif ($column->link) {
            if (is_object($tablerow) && method_exists($tablerow, "getAttributes")) {
            ...
            $value = '<a href="' . $this->parser->compileString($column->link,
                    $array) . '">' . $value . '</a>';
        }
        if (count($column->actions) > 0) {
            ...
            $value = \View::make('rapyd::datagrid.actions',
                array('uri' => $column->uri, 'id' => $keyvalue, 'actions' => $column->actions));
        }

        return $value;
    }

    public function addRoute($route, $label = '<i class="fa fa-pencil"></i>')
    {
        return $this->add('_edit', $label, false)->route($route);
    }
    ...
    protected function sanitize($string)
    {
        ...
        return Config::get('rapyd.sanitize.num_characters') > 0 ? str_limit($result,
            Config::get('rapyd.sanitize.num_characters')) : $result;
    }
